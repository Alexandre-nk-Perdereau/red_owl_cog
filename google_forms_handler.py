...
class GoogleFormsHandler:
    ...
    def initialize_services(self):
        ...
        try:
            ...
            print(f"[GoogleForms] GOOGLE_CREDENTIALS_JSON présent: {bool(credentials_json)}")
            
            if credentials_json:
                print("[GoogleForms] Tentative de chargement depuis GOOGLE_CREDENTIALS_JSON...")
                try:
                    credentials_info = json.loads(credentials_json)
                    print(f"[GoogleForms] JSON parsé avec succès. Type: {credentials_info.get('type')}")
                ...
                credentials_path = os.getenv("GOOGLE_CREDENTIALS_PATH", "/home/alexa/cogs/red_owl_cog/google.json")
                print(f"[GoogleForms] Recherche du fichier: {credentials_path}")
                print(f"[GoogleForms] Fichier existe: {os.path.exists(credentials_path)}")
            ...
            print(f"[GoogleForms] ERREUR lors de l'initialisation des services Google: {type(e).__name__}: {e}")
            import traceback
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(image_url) as response:
                    if response.status != 200:
                        print(f"[GoogleForms] Erreur téléchargement image: {response.status}")
                        return None
    ...
    async def create_tournament_form(self, tournament_theme, round_number, matches, vote_duration):
        ...
        try:
            ...
            for match in matches:
                if not match.get('bye'):
                    if match.get('participant1_image'):
                        image_uploads.append((match['participant1_id'], match['participant1_name'], match['participant1_image']))
                    if match.get('participant2_image'):
                        image_uploads.append((match['participant2_id'], match['participant2_name'], match['participant2_image']))
            ...
            if image_uploads:
                ...
                for (p_id, p_name, p_image), result in zip(image_uploads, upload_results):
            ...
            for _, match in enumerate(matches):
                ...
                print(f"[GoogleForms] Ajout de la question pour le match {match_index+1}: {p1_name} vs {p2_name}")
            ...
            if requests:
                print(f"[GoogleForms] Ajout de {len(requests)} questions au formulaire...")
            ...
            print(f"[GoogleForms] ERREUR HTTP lors de la création du formulaire: {error}")
            print(f"[GoogleForms] Status: {error.resp.status}, Reason: {error.resp.reason}")
    ...
    async def get_form_responses(self, form_id):
        ...
        try:
            ...
            print(f"[GoogleForms] Nombre de réponses reçues: {len(responses.get('responses', []))}")
            form = self.forms_service.forms().get(formId=form_id).execute()
            print(f"[GoogleForms] Nombre d'items dans le formulaire: {len(form.get('items', []))}")
            ...
            for idx, item in enumerate(items):
                if 'questionItem' in item:
                    ...
                    options = item['questionItem']['question'].get('choiceQuestion', {}).get('options', [])
                    print(f"[GoogleForms] Options: {[opt.get('value') for opt in options]}")
                    
                    if form_id in self.match_mappings and idx in self.match_mappings[form_id]:
            ...
            print(f"[GoogleForms] Mapping option -> participant: {option_to_participant}")
                ...
                for response_idx, response in enumerate(responses['responses']):
                    ...
                    if 'answers' in response:
                        for _, answer in response['answers'].items():
                            ...
                            if participant_name and participant_name in option_to_participant:
                                participant_id = option_to_participant[participant_name]
                                print(f"[GoogleForms] Vote pour {participant_name} (ID: {participant_id})")
                                
                                for match_idx, match_info in self.match_mappings[form_id].items():
                                    if participant_id in [match_info['p1_id'], match_info['p2_id']]:
    ...
    async def delete_form(self, form_id):
        ...
        try:
            ...
            if error.resp.status == 404:
                ...
                print(f"Erreur HTTP lors de la suppression du formulaire {form_id}: {error}")
                return False
        except Exception as e:
