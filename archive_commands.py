...
class ArchiveCommands:
    ...
    async def transfer_message(self, message: discord.Message, target_channel: discord.TextChannel | discord.Thread):
        ...
        if message.reference and isinstance(message.reference.resolved, discord.Message):
            ...
            reply_content = reply_msg.content[:100] + ('...' if len(reply_msg.content) > 100 else '')
            embed.add_field(name=f"↪️ Répond à {reply_msg.author.display_name}", value=f"> {reply_content}\n> [Voir l'original]({reply_msg.jump_url})", inline=False)
        elif message.reference:
             embed.add_field(name="↪️ Répond à un message", value="_(Message original inaccessible ou supprimé)_", inline=False)
            ...
            for i, att in enumerate(message.attachments):
                 ...
                 try:
                    if att.size < 8 * 1024 * 1024: # 8 MiB
                        ...
                        attachment_texts.append(f"- [{att.filename}]({att.url}) (Fichier trop volumineux > 8MB)")
        ...
        if message.stickers:
            ...
            embed.add_field(name="✨ Stickers", value=", ".join(sticker_names), inline=False)
        ...
        if not embed.description and not embed.fields and not embed.image and not message.stickers:
             if attached_files:
        ...
        try:
            ...
            await target_channel.send(f"> :warning: Échec du transfert du message {message.jump_url}. Erreur: {e}")
            return None
    ...
    async def transfer(self, ctx: commands.Context, channel_link: str):
        ...
        if isinstance(destination_channel, discord.TextChannel) and not dest_perms.create_public_threads:
             missing_perms.append("Créer des fils publics") # Even private source threads become public
            ...
            async for message in source_channel.history(limit=None, oldest_first=True):
                ...
                try:
                    ...
                    if message.thread:
                        ...
                        source_thread_perms = source_thread.permissions_for(source_thread.guild.me)
                        if not source_thread_perms.read_message_history:
                             await ctx.send(f"> :warning: Impossible de lire l'historique du fil public {source_thread.mention} ({source_thread.name}). Il sera ignoré.")
                             error_count += 1
                             continue
                        ...
                        if sent_bot_message:
                            try:
                                target_thread = await sent_bot_message.create_thread(name=f"{source_thread.name}")
                                thread_base_count += 1
                                await status_message.edit(content=f"⏳ Transfert principal... Message {msg_count}, Fil {thread_base_count} créé: {target_thread.name}")
                                ...
                                await ctx.send(f"> :warning: Permission refusée pour créer un fil pour le message {message.jump_url}.")
                                error_count += 1
                        else:
                             await ctx.send(f"> :information_source: Le message original {message.jump_url} (qui avait un fil) a été sauté ou a échoué, impossible de créer le fil correspondant lié.")
                             error_count += 1
                        ...
                        if target_thread:
                            ...
                            async for thread_message in source_thread.history(limit=None, oldest_first=True):
                                ...
                                if thread_msg_count % 20 == 0:
                                    await status_message.edit(content=f"⏳ Transfert principal... Message {msg_count}, Fil {target_thread.name} ({thread_msg_count} messages)")


                except Exception as e_msg:
                    error_count += 1
                    await ctx.send(f"> :warning: Erreur lors du traitement du message principal {message.jump_url}: {e_msg}")

                if msg_count % 20 == 0:
                     await status_message.edit(content=f"⏳ Transfert principal... {msg_count} messages traités.")

            await status_message.edit(content=f"✅ Transfert principal terminé ({msg_count} messages). Recherche d'autres fils...")

        except discord.Forbidden:
            await status_message.edit(content=f"❌ Erreur: Le bot n'a pas les permissions suffisantes pour lire l'historique du canal source ({source_channel.mention}).")
            return
        except discord.HTTPException as http_e:
             await status_message.edit(content=f"❌ Erreur HTTP majeure pendant le transfert principal: {http_e}. Le transfert est interrompu.")
             return
        except Exception as e:
            await status_message.edit(content=f"❌ Une erreur inattendue est survenue pendant le transfert principal: {e}. Tentative de transfert des fils restants...")
            error_count +=1

        await status_message.edit(content=f"⏳ Recherche des fils actifs et archivés dans {source_channel.mention}...")
        ...
        try:
            ...
            await ctx.send(f"> :warning: Le bot n'a pas la permission de voir les fils archivés dans {source_channel.mention}.")
        except discord.HTTPException as e:
            await ctx.send(f"> :warning: Erreur HTTP lors de la récupération des fils archivés: {e}")
        except AttributeError:
            await ctx.send(f"> :information_source: Le type de canal {type(source_channel).__name__} ne supporte pas la récupération de fils archivés.")
        except Exception as e:
             await ctx.send(f"> :warning: Erreur inattendue lors de la récupération des fils archivés: {e}")


        await status_message.edit(content=f"⏳ Transfert des {len(all_source_threads)} fils trouvés (actifs et archivés)...")
        ...
        for source_thread in all_source_threads:
            ...
            try:
                ...
                if not perms.read_message_history:
                    await ctx.send(f"> :warning: Ne peut pas lire l'historique du fil '{source_thread.name}' ({source_thread.mention}). Il sera ignoré.")
                    error_count += 1
                    continue
            except Exception as e:
                await ctx.send(f"> :warning: Erreur de vérification des permissions pour le fil '{source_thread.name}': {e}. Il sera ignoré.")
                error_count += 1
                continue

            target_thread = None
            try:
                placeholder_msg = await destination_channel.send(f"🧵 Transfert du fil: **{source_thread.name}** (Original: {source_thread.mention})")
                target_thread = await destination_channel.create_thread(name=f"{source_thread.name}", message=placeholder_msg)
                thread_extra_count += 1
                await status_message.edit(content=f"⏳ Transfert des fils... Fil {thread_extra_count}/{processed_in_phase2} créé: {target_thread.name}")
                await asyncio.sleep(self.THREAD_DELAY)
            except discord.HTTPException as thread_e:
                 await ctx.send(f"> :warning: Impossible de créer le fil de destination pour '{source_thread.name}': {thread_e}")
                 error_count += 1
                 continue
            except discord.Forbidden:
                 await ctx.send(f"> :warning: Permission refusée pour créer des fils dans {destination_channel.mention}. Arrêt du transfert des fils restants.")
                 error_count += 1
                 break
            except Exception as e:
                await ctx.send(f"> :warning: Erreur inattendue lors de la création du fil pour '{source_thread.name}': {e}")
                error_count += 1
                continue
            if target_thread:
                thread_msg_count = 0
                async for thread_message in source_thread.history(limit=None, oldest_first=True):
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                    if thread_msg_count % 20 == 0:
                        await status_message.edit(content=f"⏳ Transfert des fils... Fil {target_thread.name} ({thread_msg_count} messages)")
    ...
    async def parse_thread_link(self, link: str):
        ...
        match = re.match(r"https?://(?:ptb\.|canary\.)?discord\.com/channels/(\d+)/(\d+)(?:/\d+)?/?$", link)
        if match:
    ...
    async def fetch_thread_safe(self, guild_id: int, thread_id: int):
        ...
        if not guild:
            ...
            return None, f"Le bot n'est pas membre du serveur (ID: {guild_id}) contenant ce fil."

        try:
            # Tenter de récupérer le fil via l'API (plus fiable)
            thread = await guild.fetch_channel(thread_id) # fetch_channel peut aussi récupérer des fils
            if not isinstance(thread, discord.Thread):
                 ...
                 if not thread:
                     return None, f"Impossible de trouver un fil avec l'ID {thread_id} sur le serveur '{guild.name}'. Le lien est-il correct ?"

        except discord.NotFound:
            # Essayer de le trouver dans le cache du guild s'il n'est pas trouvé via fetch
            thread = guild.get_thread(thread_id)
            if not thread:
                print(f"Fetch Thread: Thread {thread_id} not found in guild {guild.name}")
                return None, f"Impossible de trouver le fil (ID: {thread_id}) sur le serveur '{guild.name}'. Vérifiez le lien."
        except discord.Forbidden:
            print(f"Fetch Thread: Forbidden to fetch thread {thread_id} in guild {guild.name}")
            return None, f"Le bot n'a pas la permission de voir le fil (ID: {thread_id}) sur le serveur '{guild.name}'."
        except Exception as e:
            print(f"Fetch Thread: Unexpected error fetching thread {thread_id} in guild {guild.name}: {e}")
            return None, f"Une erreur inattendue est survenue lors de la récupération du fil : {e}"

        # Vérifier les permissions de lecture dans le fil
        perms = thread.permissions_for(guild.me)
        if not perms.read_message_history:
            print(f"Fetch Thread: Missing read_message_history for thread {thread.name} ({thread_id})")
            return None, f"Le bot n'a pas la permission de 'Lire l'historique des messages' dans le fil '{thread.name}'."
    ...
    async def create_thread_transcript(self, thread: discord.Thread):
        ...
        transcript_lines = []
        transcript_lines.append(f"--- Début de la transcription du fil: {thread.name} ({thread.id}) ---")
        transcript_lines.append(f"--- Serveur: {thread.guild.name} ({thread.guild.id}) ---")
        transcript_lines.append(f"--- Créé le: {thread.created_at.strftime('%d/%m/%Y %H:%M:%S UTC')} ---")
        ...
        try:
            async for message in thread.history(limit=None, oldest_first=True):
                ...
                if message.stickers:
                    sticker_info = f"  (Sticker(s): {', '.join(s.name for s in message.stickers)})"
        ...
        transcript_lines.append(f"--- Fin de la transcription ({message_count} message(s)) ---")
        return "\n".join(transcript_lines)
    ...
    async def generate_transcript_from_link(self, thread_link: str):
        ...
        if not guild_id or not thread_id:
            return None, "Le lien fourni n'est pas un lien de fil Discord valide ou reconnaissable (format attendu: .../channels/SERVEUR_ID/FIL_ID)."
        ...
        try:
            ...
            return file_path, discord_filename # Retourne le chemin local et le nom pour Discord

        except Exception as e:
            print(f"Generate Transcript: Error creating temp file: {e}")
            return None, f"Une erreur est survenue lors de la création du fichier de transcription : {e}"
